...
import { WaterLevelThresholds, DEFAULT_THRESHOLDS } from '../types/waterData';
...
export function useWaterData() {
  ...
  const [thresholds, setThresholds] = useState<WaterLevelThresholds>(() => {
    const saved = localStorage.getItem('hydroguard_water_thresholds');
    if (saved) {
      try {
        return JSON.parse(saved);
      } catch (err) {
        console.error('Invalid saved thresholds, using defaults', err);
      }
    }
...
useEffect(() => {
    const handleStorageChange = (e: StorageEvent) => {
      if (e.key === 'hydroguard_water_thresholds' && e.newValue) {
        try {
  ...
  // Use saved/user-configured values instead of hard-coded numbers
  if (value < thresholds.minSafe) {
    return 'critical';           // low water = critical (drought risk)
  }
  ...
  return 'critical';             // above critical = flood danger
};;
