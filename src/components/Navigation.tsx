...
import { HomeIcon, LayoutDashboardIcon, MessageSquareIcon, SettingsIcon, InfoIcon } from 'lucide-react';

type PageType = 'home' | 'dashboard' | 'chat' | 'settings' | 'about';
    ...
    <nav className="fixed bottom-0 left-0 right-0 bg-slate-900/90 backdrop-blur-md border-t border-indigo-500/20 px-6 py-2 z-50 safe-area-bottom shadow-[0_-4px_12px_-2px_rgba(99,102,241,0.15)]">
      <div className="max-w-md mx-auto flex justify-around items-end">
        ...
        <button
          ...
          className={`flex flex-col items-center gap-1 p-2 transition-all duration-300 active:scale-95 relative ${currentPage === 'about' ? 'text-indigo-400' : 'text-slate-500 hover:text-slate-400'}`}
          aria-label="About"
        >
          {currentPage === 'about' && (
            <span className="absolute -top-1 left-1/2 -translate-x-1/2 w-8 h-0.5 bg-indigo-400 rounded-full animate-slide-in"></span>
          ...
          <InfoIcon
            className={`w-6 h-6 transition-all duration-300 ${currentPage === 'about' ? 'fill-current/20 stroke-[2.5px] scale-110' : 'stroke-2'}`}
          />
